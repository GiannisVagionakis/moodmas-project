...
// Calculate happiness score from emotions
export const calculateHappinessScore = (emotions: EmotionScores): number => {
  // Weighted calculation: happy is positive, sad/angry/fearful are negative
  const positive = emotions.happy * 100 + emotions.surprised * 50;
  const negative = (emotions.sad + emotions.angry + emotions.fearful + emotions.disgusted) * 50;
