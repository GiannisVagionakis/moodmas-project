...
// Simple Christmas melody using Web Audio API
const createChristmasMelody = () => {
  ...
  try {
    ...
    // Christmas melody notes (Jingle Bells pattern)
    const melody = [
...
export const BackgroundMusic: React.FC = () => {
  ...
  const playMelody = () => {
    ...
    const tempo = 0.4; // Speed multiplier
    ...
    const playNote = (note: string, duration: number, startTime: number) => {
      ...
      gainNode.gain.exponentialRampToValueAtTime(0.01, startTime + duration * tempo);
  ...
  return (
    <button
      onClick={toggleMusic}
      className={`fixed bottom-4 right-4 z-40 glass-card px-4 py-3 rounded-full hover:scale-105 transition-all flex items-center gap-2 ${
        isPlaying ? 'bg-green-500/20 border-green-500/30' : 'bg-white/10'
      }`}
